...
/// Education section of the portfolio website with animations and elegant styling.
...
class _EducationSectionState extends State<EducationSection> with TickerProviderStateMixin {
  double _opacity = 0.0;
  late AnimationController _controller;
  late Animation<Offset> _slideAnimation;
  bool _isHovered = false; // For section title hover
  late AnimationController _particleController;
  final int _particleCount = 7;
  late List<_Particle> _particles;

  // Load education data from JSON with enhanced error handling
  Future<List<Map<String, dynamic>>> _loadEducationData() async {
    return await ErrorHandler.safeAsyncOperation<List<Map<String, dynamic>>>(
      operation: () async {
        final String jsonString = await rootBundle.loadString('assets/data/education.json');
        if (jsonString.trim().isEmpty) {
          throw Exception('Education data is empty.');
        }
      ...
      operationName: 'LoadEducationData',
      fallbackValue: kIsWeb ? [
        {
          "institution": "University of Advanced Studies",
          "degree": "Master of Science in Computer Engineering",
          "duration": "2018 - 2020",
          "description": "Focused on AI and Machine Learning. Thesis on NLP."
        },
        {
          "institution": "Institute of Technology",
          "degree": "Bachelor of Science in Software Development",
          "duration": "2014 - 2018",
          "description": "Graduated with honors. Capstone project on mobile app development."
        }
      ] : null,
    ) ?? (throw Exception('Failed to load education data. Please check your data file.'));
  }
  ...
  void initState() {
    ...
    _slideAnimation = Tween<Offset>(
      begin: const Offset(0.15, 0), // Slide from right for variety
      end: Offset.zero,
  ...
  void dispose() {
    ...
    ErrorHandler.safeDisposeControllers(
      ...
      names: ['EducationMainController', 'EducationParticleController'],
    );
    super.dispose();
  }
                        ...
                        child: AnimatedBuilder(
                          ...
                          builder: (context, child) {
                            ...
                            return Transform.translate(
                              offset: Offset(cos(t * 2 * pi + pi / 3) * 7, sin(t * 2 * pi + pi / 3) * 7),
                              child: Opacity(
                                opacity: 0.08,
                          ...
                          return Center(
                            child: Padding(
                              ...
                              child: Column(
                                mainAxisAlignment: MainAxisAlignment.center,
                                children: [
                                  Icon(Icons.school_outlined, color: theme.colorScheme.secondary, size: 48),
                                  const SizedBox(height: 12),
                                  Text(
                                    'No education to display at the moment.',
                                    style: theme.textTheme.titleLarge?.copyWith(
...
class _DynamicEducationGrid extends StatelessWidget {
  final List<Map<String, dynamic>> educationList;
  final int columns;
  final double spacing;

  const _DynamicEducationGrid({
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < educationList.length; i++) {
      columnItems[i % columns].add(educationList[i]);
    }
              ...
              child: Column(
                ...
                children: columnItems[columnIndex].asMap().entries.map((entry) {
                  ...
                  return Padding(
                    ...
                    child: _EducationCard(edu: entry.value)
                      .animate()
...
/// Card widget for displaying education data with enhanced UI and hover effect.
...
class _EducationCardState extends State<_EducationCard> {
  ...
  bool _isDetailsExpanded = false; // Single toggle for both description and skills
        ...
        child: Card(
          ...
          child: Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                // Institution with colored accent
                Row(
                ...
                if (isMobile) ...[
                  AnimatedCrossFade(
                    duration: const Duration(milliseconds: 250),
                    crossFadeState: _isDetailsExpanded ? CrossFadeState.showFirst : CrossFadeState.showSecond,
                    firstChild: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        if (widget.edu['description'] != null && widget.edu['description']!.isNotEmpty)
                          Text(
                        ...
                        const SizedBox(height: 12),
                        if (widget.edu['skills'] != null && widget.edu['skills'] is List)
                          Wrap(
                            spacing: 8,
                            runSpacing: 8,
                            children: (widget.edu['skills'] as List).map((skill) => Chip(
                              label: Text(
                                skill.toString(),
                                style: TextStyle(
                                  fontSize: 12,
                  ...
                  Align(
                    ...
                    child: TextButton.icon(
                      icon: Icon(_isDetailsExpanded ? Icons.expand_less : Icons.expand_more),
                      label: Text(_isDetailsExpanded ? 'Hide Details' : 'Show Details'),
                      onPressed: () => setState(() => _isDetailsExpanded = !_isDetailsExpanded),
                  ...
                  // On desktop, show both description and skills as before
                  if (widget.edu['description'] != null && widget.edu['description']!.isNotEmpty)
                    Text(
                      widget.edu['description']!,
                      style: theme.textTheme.bodyMedium?.copyWith(
...
// Particle and painter classes (copied from experience_section.dart)
class _Particle {
...
class _ParticlePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final paint = Paint()
      ..color = color.withAlpha(35) // Adjusted alpha for consistency
